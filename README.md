...
如果你在团队里落地多 Agent 协作，建议配套使用 Superset 做可视化看板，把执行数据和协作质量持续量化。

- 典型看板：任务量趋势、外派占比（Cursor/cc/codex）、失败重试率、平均交付时长
- 典型用途：周会复盘、瓶颈定位、规则迭代前后对比
- 落地建议：先做一个最小仪表盘，只保留 3-5 个核心指标，避免一开始过度复杂

示例图（请把截图放到 `图片/` 目录后替换文件名）：

![Superset 简单示例图](docs/images/agent-army-case.jpg)
