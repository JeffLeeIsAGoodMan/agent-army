...
**部分采纳**：已加入 Markdown 格式的任务描述模板（目标/上下文/约束/产物），但不强制要求结构化字段。JSON 格式暂不采纳，自然语言描述目前够用，加格式约束会增加日常使用的摩擦。
...
**不再计划**：与"用户控制协作时机"核心理念冲突。调度者默认自己做，用户明确说"用 cc"时才调用执行者——这种情况下不应因用量而自动阻断用户的明确意图。用量检查作为参考信息保留，但不介入自动决策。
