...
cc 的参数改了，或者 codex 的调用方式变了，你得去改 Rule。Rule 是全局的，改错了影响所有对话。（后来这些调用模板被移到了 Skill 子文档 cc-reference.md 和 codex-reference.md 中。）
...
主文件始终加载，子文档按需引用。cc-reference.md 和 codex-reference.md 分别包含各自的调用模板和参数说明，collaboration.md 包含协作文件规范和交接模板（不含调用模板）。这样 token 消耗和信息精度都得到了优化。
...
**目录初始化**：调度者应确保目录存在，不存在则创建（`mkdir -p`）。不需要用户手动创建，也不需要预先存在——每次调用前确保目录在就行。
...
```mermaid
pie title 历史执行占比（示例数据，2026-04 内部统计）
  "Cursor（调度者）" : 95.7
  "cc（执行者）" : 2.9
  "codex（顾问）" : 1.4
```
...
数据 review 不应该只做一次。每次改完规则后，怎么知道改动有没有效？靠直觉不靠谱——上一轮就是靠直觉觉得"规则写了就会执行"。

解决方式是设计一套**标准测试任务集**，覆盖不同场景：

```mermaid
flowchart LR
  改["改规则 / Skill"] --> 跑["跑 2-3 个测试任务"]
  跑 --> 记["记录实际分工占比"]
  记 --> 比["对比期望分工"]
  比 --> 改
...
每个测试任务定义了：
- **任务描述**：贴给 `/army` 的具体文本
- **期望分工**：谁该做什么、Cursor 最多做几步
- **检查点**：有没有自己做、有没有并行、有没有写文件

关键指标是 **Cursor 自己做的步骤占比 ≤ 40%**。超过这个比例说明规则没有有效触发外派。

详见 [docs/test-scenarios.md](test-scenarios.md)。

### 第一次实测：闭环验证

测试任务集设计好后，真正跑了一次。挑了三个任务：T1（批量格式统一，4 个文件）、T2（多文件 Review，3 个文档）、T5（简单小改，对照组）。

实测过程本身就是在用 `/army` 协作流程：

```
T1：Cursor 判断 → 出计划 → 等确认 → 写任务文档到 _agent_work/plans/
   → 并行启动 2 个 cc 实例（格式修改 + 引用检查）→ 验收产物
   耗时 ~68 秒，Cursor 只做了验收

T2：Cursor 判断 → 并行启动 3 个 cc 实例（分别 review 3 个文档）→ 汇总评分
   耗时 ~55 秒，Cursor 只做了汇总
   意外收获：review 发现了 concept.md 自身的 4 处不一致

T5：Cursor 直接改了一行标题 → 完成
   没有启动 cc/codex，没有过度拆解
```
...
block-beta
  columns 3
  space header1["指标"] header2["结果"]
  A["Cursor 步骤占比"] A1["33%（≤40% ✅）"]
  B["cc 派发"] B1["全部派出 ✅"]
  C["并行执行"] C1["T1×2 T2×3 ✅"]
  D["产物写文件"] D1["全部 ✅"]
  E["健康度"] E1["5/5 × 3 任务"]
...
三个任务全部 5/5 健康度。更关键的是用量消耗：cc（Kimi）从 2% 到 2%——几乎没动。Cursor 的 quota 主要花在调度和验收上，重活全是 cc 在做。

这次实测验证了两件事：

1. **铁律生效了**。T1 和 T2 都触发了"≥3 文件 → 派 cc"的规则，Cursor 没有自己逐个文件去改或去 review。而 T5 这种简单任务，Cursor 正确判断了不需要外派。
2. **测试任务集的设计是有效的**。标准场景 + 期望分工 + 检查点这套结构，能快速暴露"AI 自己做了不该做的事"。

但也发现了一个实际问题：**TUN 可用性检测的超时设置偏紧**。第一次检测时 3 秒超时导致误判 codex 不可用，实际上网络只是短暂波动。后来改为 5 秒后恢复正常。这类"基础设施层面的小坑"只有实测才能发现。

**教训：测试任务集不只是验证分工逻辑——它还能暴露工具链本身的脆弱点。设计再完善的流程，不跑一遍就不知道哪里会卡。**
